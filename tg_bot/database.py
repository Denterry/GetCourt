...
class BotDB:
    ...
    def add_court(self, user_id, name, image_id, address, lat, lng):
        ...
        """
        self.cursor.execute("INSERT INTO courts (user_id, name, image_id, address, latitude, longitude, players, green_player, yellow_player, red_player, years) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def get_courts_nearby(self, latitude: float, longitude: float, radius: float = 5.0) ->\
        ...
        self.cursor.execute(
            "SELECT *, (acos(sin(radians(?))*sin(radians(latitude)) + cos(radians(?))*cos(radians(latitude))*cos(radians(longitude)-radians(?)))*6371) AS distance FROM courts WHERE distance<3 ORDER BY distance",
    ...
    def get_nearby_courts(self, user_location):
        ...
        """
        self.cursor.execute('SELECT * FROM courts')
    ...
    def get_nearest_court(self, latitude, longitude):
        ...
        """
        self.cursor.execute(
            "SELECT *, (acos(sin(radians(?))*sin(radians(latitude)) + cos(radians(?))*cos(radians(latitude))*cos(radians(longitude)-radians(?)))*6371) AS distance FROM courts WHERE distance<0.05 ORDER BY distance",
            (latitude, latitude, longitude,))
    ...
    def get_court_by_id(self, court_id: int) -> Tuple[int, str, str, str, float, float, int, int, int, int, int]:
        ...
        """
        self.cursor.execute("SELECT * FROM courts WHERE id=?", (court_id,))
    ...
    def add_player_on_court(self, user_id, court_id):
        ...
        self.cursor.execute("UPDATE courts SET players=? WHERE id=?", ((court_info[7] + 1), court_id,))
        if (user_info[4] == "lovely_play"):
            self.cursor.execute("UPDATE courts SET green_player=? WHERE id=?", ((court_info[8] + 1), court_id,))
        elif (user_info[4] == "middle_play"):
            self.cursor.execute("UPDATE courts SET yellow_player=? WHERE id=?", ((court_info[9] + 1), court_id,))
        else:
            self.cursor.execute("UPDATE courts SET red_player=? WHERE id=?", ((court_info[10] + 1), court_id,))

        self.cursor.execute("UPDATE courts SET years=? WHERE id=?", (user_info[5], court_id,))
        return self.conn.commit()
    ...
    def add_basketball_event(self, admin_id, eventname, acsess, levels, address,
        ...
        """
        self.cursor.execute(
            "INSERT INTO `events` (`admin_id`, `name`, `acsess`, `players_lvls`, `address`, `latitude`, `longitude`, `date`, `time`, `description`, `seted`, `continued`, `finished`, `image_id`, `players`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (admin_id, eventname, acsess, levels, address, latitude, longitude, date, time, description, 0, 0, 0, 0,
             1))
        return self.conn.commit()

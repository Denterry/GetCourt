...
async def process_start_command(message: Message):
    ...
    await message.answer(text='Привет!\n'
                              'Этот бот - решение твоей насущной проблемы, здесь ты найдешь единомышленников по командным видам спорта и площадки для занятия!\n\n'
...
async def process_help_command(message: Message):
    ...
    await message.answer(text='Этот бот создан для объединения спортивного комьюнити и решения проблемы, одиночного занятия спортом!\n'
                                  'Краткий экскурс по интерфейсу:\n'
                                  '"⛹🏿‍♂️ Найти площадку поблизости" - нажав на нее, вы сможите согласно своей геолокации или выбранной точке на карте, найти ближайшую спортивную площадку со всеми характеристиками;\n'
                                  '"🗑️ Добавить новую площадку" - нажав на нее, вы сможете в несколько шагов создать новую спортивную площадку;\n'
...
async def process_support_help(callback: CallbackQuery):
    await callback.message.answer('Этот бот создан для объединения спортивного комьюнити и решения проблемы, одиночного занятия спортом!\n'
                                  'Краткий экскурс по интерфейсу:\n'
                                  '"⛹🏿‍♂️ Найти площадку поблизости" - нажав на нее, вы сможите согласно своей геолокации или выбранной точке на карте, найти ближайшую спортивную площадку со всеми характеристиками;\n'
                                  '"🗑️ Добавить новую площадку" - нажав на нее, вы сможете в несколько шагов создать новую спортивную площадку;\n'
                                  '"🧾 Показать мероприятия" - нажав на нее, вам будет предоставлен полный список мероприятий существующих на данный момент\n'
    ...
    try:
        ...
        if (nearest_courts_data == None or nearest_courts_data == []):
            ...
            await message.answer_photo(
                ...
                reply_markup=pagination.create_pagination_keyboard(
                    'backward',
...
@router.message(StateFilter(AddEvent.eventname), F.content_type == ContentType.TEXT)
...
async def open_profil(callback: CallbackQuery):
    ...
    username_botton: InlineKeyboardButton = InlineKeyboardButton(
        text=f'Имя: {user_info[2]}',
    ...
    game_level_botton: InlineKeyboardButton = InlineKeyboardButton(
        text=f'Уровень: {lvl}',
        callback_data='lvl')
    years_exprs_botton: InlineKeyboardButton = InlineKeyboardButton(
        text=f'Лет в баскетболе: {user_info[5]}',
        callback_data='years')
    keyboard: InlineKeyboardMarkup = InlineKeyboardMarkup(
        inline_keyboard=[[username_botton, game_level_botton],
                         [years_exprs_botton]])
    await callback.message.answer(
        text='Какие данные желаете изменить:',
        reply_markup=keyboard)
    await callback.answer()

class ChangeDataUser(StatesGroup):
    username=State()
    lvl=State()
    years=State()

@router.callback_query(Text(text=['username']), StateFilter(default_state))
async def open_profil_username(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        text='Введите новое имя:')
    await state.set_state(ChangeDataUser.username)
    await callback.answer()

@router.message(StateFilter(ChangeDataUser.username), F.content_type == ContentType.TEXT)
async def change_profil_username(message: Message, state: FSMContext):
    await state.update_data(username=message.text)
    get_user_info_state = await state.get_data()
    BotDB.change_username(get_user_info_state["username"], message.from_user.id)
    await message.answer(
        text='Имя успешно изменено!')
    await state.clear()

@router.callback_query(Text(text=['lvl']), StateFilter(default_state))
async def open_profil_lvl(callback: CallbackQuery, state: FSMContext):
    love_botton: InlineKeyboardButton = InlineKeyboardButton(
        text='🟢 Новичок',
        callback_data='lovely_play')
    middle_botton: InlineKeyboardButton = InlineKeyboardButton(
    ...
    keyboard: InlineKeyboardMarkup = InlineKeyboardMarkup(
        inline_keyboard=[[love_botton],
                         ...
                         [extra_botton]])
    await callback.message.answer(
        text='Выберите ваш новый уровень:',
        reply_markup=keyboard)
    await state.set_state(ChangeDataUser.lvl)
    await callback.answer()

@router.callback_query(Text(text=['lovely_play', 'middle_play', 'extra_play']), StateFilter(ChangeDataUser.lvl))
async def change_profil_username(callback: CallbackQuery, state: FSMContext):
    await state.update_data(lvl=callback.data)
    get_user_info_state = await state.get_data()
    BotDB.change_game_level(get_user_info_state["lvl"], callback.from_user.id)
    await callback.message.answer(
        text='Уровень успешно изменён!')
    await state.clear()
    await callback.answer()

@router.callback_query(Text(text=['years']), StateFilter(default_state))
async def open_profil_years(callback: CallbackQuery, state: FSMContext):
    one_botton: InlineKeyboardButton = InlineKeyboardButton(
        text='<= 1 год',
        callback_data='one')
    two_botton: InlineKeyboardButton = InlineKeyboardButton(
    ...
    keyboard: InlineKeyboardMarkup = InlineKeyboardMarkup(
        inline_keyboard=[[one_botton],
                         ...
                         [eight_botton]])
    await callback.message.answer(
        text='Выберите количество лет в баскетболе:',
        reply_markup=keyboard)
    await state.set_state(ChangeDataUser.years)
    await callback.answer()

@router.callback_query(Text(text=['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight']), StateFilter(ChangeDataUser.years))
async def change_years_exprs(callback: CallbackQuery, state: FSMContext):
    await state.update_data(years=callback.data)
    user_data = await state.get_data()
    years = 0

    if (user_data["years"] == 'one'):
        years = 1
    elif (user_data["years"] == 'two'):
        years = 2
    elif (user_data["years"] == 'three'):
        years = 3
    elif (user_data["years"] == 'four'):
        years = 4
    elif (user_data["years"] == 'five'):
        years = 5
    elif (user_data["years"] == 'six'):
        years = 6
    elif (user_data["years"] == 'seven'):
        years = 7
    else:
        years = 8
    BotDB.change_years_exprs(years, callback.from_user.id)
    await callback.message.answer(
        text='Год успешно изменён!')
    await state.clear()
    await callback.answer()
